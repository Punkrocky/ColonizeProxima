...
#{
  ...
  #{
    ...
    || GroundTileMap.get_cell_atlas_coords(TileCoord) != Vector2i(3,0) # Grass ground tiles
...
#{
  ...
  #{
    ...
    #{
      ...
      BuildingTileMap.set_cell(BuildingTileMap.local_to_map(position - BuildingTileMap.position), 1, Vector2i(1,0));
...
#{
  produced_resource.emit(Enums.ResourceIconType.FOOD);
...
#{
  ...
  BuildingTileMap.set_cell(TileMapCoord, 0, Vector2i(1,0));
